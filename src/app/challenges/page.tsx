...
import { Challenge } from '@/lib/level'; 
        ...
        <Table>
          ...
          <TableBody>
            {challenges.map((challenge: Challenge) => {
              const createdAtResult = formatDistanceToNow(new Date(challenge.createdAt), {
                addSuffix: true,
              });
              ...
              return (
                <Link href={`/challenges/${challenge.id}`} key={challenge.id} legacyBehavior>
                  <TableRow className="cursor-pointer">
                    <TableCell className="w-[40%]">{challenge.title}</TableCell>
                    <TableCell className="w-[40%]">{challenge.description}</TableCell>
                    <TableCell className="w-[40%]">{levelDescription}</TableCell>
